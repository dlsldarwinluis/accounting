...
<head>
  ...
  <link rel="stylesheet" href="/IT3AHCTS/A&FD/accountingandfinance/css/payroll-initial-list.css">
</head>
<body>
  ...
  <div id="forapprovalContainer">
      <div id="styleForm">
        <div id="textForm" class="inline-Cont">Payroll Initial List</div>
        <div id="noteForm" class="inline-Cont"><form action="payroll-initial-list-search.php" method="post" id="form-search" class="inline-Cont">
          ...
          <table id="forapprovalTable" align="center">
              ...
              <tbody>
                  <?php
                    ...
                    $Today = date('F');
                    $query = mysqli_query($con, "SELECT * FROM hr_employees WHERE dateofrelease LIKE '%".$Today."%' AND payrollstatus_af='' AND lastname LIKE '%".$search."%' || dateofrelease LIKE '%".$Today."%' AND payrollstatus_af='' AND middlename LIKE '%".$search."%' || dateofrelease LIKE '%".$Today."%' AND payrollstatus_af='' AND firstname LIKE '%".$search."%'");
                ...
                <tbody>
                    <tr>
                        ...
                        <td><form action="" method="post" id="viewFunction">
                            <button type="submit" id="submitView" name="submitView"><img src="/IT3AHCTS/A&FD/accountingandfinance/icons/view.png" class="tableIcons"><input type="hidden" name="requestId" id="requestId" value="<?php echo $row["id"] ?>"></button>
                        </form></td>
                        <td><form action="" method="post" id="computeFunction">
                            <button type="submit" id="submitCompute" name="submitCompute"><img src="/IT3AHCTS/A&FD/accountingandfinance/icons/compute.png" class="tableIcons"><input type="hidden" name="requestId" id="requestId" value="<?php echo $row["id"] ?>"><input type="hidden" name="requestName" id="requestName" value="<?php echo $row["firstname"] . " " . $row["middlename"] . " " . $row["lastname"] ?>"></button>
                        </form></td>
                    </tr>
                </tbody>
                            <?php
  ...
  <?php
    if(isset($_POST["submitView"])){
        ...
        $select = mysqli_query($con, "SELECT * FROM hr_employees WHERE id='$id'");
        ...
        <div id="modal">
            ...
            <table id="modalContent" align="center">
                ...
                <tr>
                    ...
                    <td><?php echo $row["totalrenderedhours"] ?> hrs</td>
                </tr>
            </table>
        </div>
    </div>
  ...
  ?>
  <?php
    if(isset($_POST["submitCompute"])){
        $id = $_POST["requestId"];
        $name = $_POST["requestName"];

        $employee = mysqli_query($con, "SELECT * FROM hr_employees WHERE id='$id'");
        while($employeedata = mysqli_fetch_assoc($employee)){
            $position = $employeedata["position"];
            $empdepartment = $employeedata["department"];
            $renderedhours = $employeedata["totalrenderedhours"];
            $mail = $employeedata["email1"];
            $release = $employeedata["dateofrelease"];

            $findrate = mysqli_query($con, "SELECT * FROM af_payrollvariables WHERE payrollposition = '$position'");
            while($ratedata = mysqli_fetch_assoc($findrate)){
                $rate = $ratedata["payrollrate"];
                $addition = $ratedata["payrolladdition"];
                $tax = $ratedata["payrolltax"];
                $taxrate = 0.10 * $tax;

                $initialsalary = $rate * $renderedhours;
                $taxated = $initialsalary * $taxrate;
                $salaryone = ($initialsalary - $taxated);
                $salarytwo = $salaryone + $addition;
                
            }
        }
        $finddeduction = mysqli_query($con, "SELECT * FROM af_loan WHERE requestname='$name' AND requeststatus='APPROVED'");
        $countdeduc = mysqli_num_rows($finddeduction);

        if(!empty($countdeduc)){
            while($loandatas = mysqli_fetch_assoc($finddeduction)){
                $employeededuction = $loandatas["loanamount"] * 0.10;
            }
        }
        else{
            $employeededuction = 0;
        }
        $finalsalary = $salarytwo - $employeededuction;
        $updateloan = $loan - $employeededuction;
        mysqli_query($con, "INSERT INTO af_payroll (employeefullname, employeedepartment, employeeposition, employeemail, employeerenderedhours, employeededuction, employeetax, employeeaddition, employeeinitialsalary, employeetaxated, employeefinalsalary, employeepayrelease, employeestatus) VALUES ('$name', '$empdepartment', '$position', '$mail', '$renderedhours', '$employeededuction', '$tax', '$addition', '$initialsalary', '$taxated', '$finalsalary', '$release', 'COMPUTED')");
        mysqli_query($con, "UPDATE af_loan
        SET loanamount = '$updateloan' WHERE requestname = '$name'");
        mysqli_query($con, "UPDATE hr_employees
        SET payrollstatus_af = 'COMPUTED' WHERE id = '$id'");
        ?>
        <script>
            alert("Successfully Computed!");
            window.location.replace(window.location.pathname + window.location.search + window.location.hash);
        </script>
        <?php
    }
