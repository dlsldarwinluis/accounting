...
<head>
  ...
  <link rel="stylesheet" href="/IT3AHCTS/A&FD/accountingandfinance/css/loan-list.css">
</head>
<body>
  ...
  <div id="forapprovalContainer">
      <div id="styleForm">
        <div id="textForm" class="inline-Cont">Loans</div>
        <div id="noteForm" class="inline-Cont"><form action="loan-list-search.php" method="post" id="form-search" class="inline-Cont">
            <input type="text" name="searchKey" id="searchKey" value="<?php echo $search ?>"  placeholder="Search">
        </form></div>
      </div>
      <div id="forapprovalContent">
          <table id="forapprovalTable" align="center">
              ...
              <tbody>
                  <?php
                    $query = mysqli_query($con, "SELECT * FROM af_loan WHERE requeststatus='' AND requestname LIKE '%".$search."%'");
                ...
                <tbody>
                    <tr>
                        ...
                        <td><form action="" method="post" id="approveFunction">
                            <button type="submit" id="submitApprove" name="submitApprove"><img src="/IT3AHCTS/A&FD/accountingandfinance/icons/approve-color.png" class="tableIcons"><input type="hidden" name="requestId" id="requestId" value="<?php echo $row["requestid"] ?>"></button>
                        </form></td>
                        <td><form action="" method="post" id="disapproveFunction">
                            <button type="submit" id="submitDisapprove" name="submitDisapprove"><img src="/IT3AHCTS/A&FD/accountingandfinance/icons/disapprove-color.png" class="tableIcons"><input type="hidden" name="requestId" id="requestId" value="<?php echo $row["requestid"] ?>"></button>
                        </form></td>
                    </tr>
                </tbody>
                            <?php
  ...
  <?php
    if(isset($_POST["submitView"])){
        ...
        $select = mysqli_query($con, "SELECT * FROM af_loan WHERE requestid='$id'");
  ...
  <?php
    if(isset($_POST["submitApprove"])){
        ...
        $sqlselect = mysqli_query($con, "SELECT * FROM af_loan WHERE requestid='$id'");

        while($rowtwo = mysqli_fetch_assoc($sqlselect)){
            $amount = $rowtwo['loanamount'];
            $hr = $rowtwo['requeststatushr'];

            if(!empty($hr)){
                $fund = mysqli_query($con, "SELECT * FROM af_fund");
                while($data = mysqli_fetch_assoc($fund)){
                    $fundAmount = $data["fundamount"];
        
                    if($fundAmount > $amount){
                        $fundTwo = $fundAmount - $amount;
        
                        mysqli_query($con, "UPDATE fund 
                        SET fundamount = '$fundTwo', funddate = '$Today'");
                        mysqli_query($con, "UPDATE af_loan
                        SET requeststatus = 'APPROVED', requestdateofassessment = '$Today'
                        Where requestid = '$id'");
                        ?>
                        <script>
                        ...
                        mysqli_query($con, "UPDATE af_loan
                        SET requeststatus = 'DISAPPROVED', requestdateofassessment = '$Today'
                        Where requestid = '$id'");
                        ?>
                        <script>
                            alert("Request Disapproved due to insufficient fund!");
                            window.location.replace(window.location.pathname + window.location.search + window.location.hash);
                        </script>
                        <?php
                    }
                ...
                ?>
                <script>
                    alert("Request is not yet assessed by HR Department! Wait for further update!");
                    window.location.replace(window.location.pathname + window.location.search + window.location.hash);
                </script>
                <?php
            }
  ...
  ?>
  <?php
    if(isset($_POST["submitDisapprove"])){
        date_default_timezone_set("Asia/Manila");
        $Today = date('F d, Y');
        $id = $_POST['requestId'];
  ...
  ?>
  <script src="/IT3AHCTS/A&FD/accountingandfinance/js/loan-list.js"></script>
</body>
</html>
<?php
    }
